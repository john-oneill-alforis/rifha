...
def classify():
    ...
    cur.execute(
        """SELECT polls_trainingcorpus.entryId, polls_trainingcorpus.text FROM polls_trainingcorpus JOIN polls_textlabels ON polls_trainingcorpus.textLabel_id = polls_textlabels.entryId WHERE polls_trainingcorpus.textLabel_id IN (22);"""
    )
    ...
    plt.title("Breach Incidents Similarity - 3rd Party Involvement")
    plt.colorbar(heatmap)
    plt.tight_layout()

    plt.savefig(toplevel_path / "plots/comparison_matrix_breach_3party.png")
    plt.show()
