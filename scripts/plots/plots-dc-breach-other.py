...
def classify():
    ...
    cur.execute(
        """SELECT polls_trainingcorpus.entryId, polls_trainingcorpus.text FROM polls_trainingcorpus JOIN polls_textlabels ON polls_trainingcorpus.textLabel_id = polls_textlabels.entryId WHERE polls_trainingcorpus.textLabel_id IN (15);"""
    )
    ...
    plt.title("Breach Incidents Similarity - No Attribution")
    plt.colorbar(heatmap)
    plt.tight_layout()

    plt.savefig(toplevel_path / "plots/comparison_matrix_breach_other.png")
    plt.show()
